...
func (pg *ProxyGen) emitPackageClause() {
	fmt.Fprintln(pg.outputBuffer, "// Code generated by proxyz. DO NOT EDIT.")
	fmt.Fprintf(pg.outputBuffer, "package %s\n", pg.outputPackageName)
}
...
`
	...
	if err := template.Must(template.New("").Parse(text)).Execute(pg.outputBuffer, data); err != nil {
		panic(err)
	}
}
...
func (pg *ProxyGen) emitMethodDeclarations() {
	...
	fmt.Fprintln(pg.outputBuffer)
	fmt.Fprintf(pg.outputBuffer, "func (p *%s) XxxNumberOfMethods() int { return %d }\n",
...
`
	...
	if err := template.Must(template.New("").Funcs(funcMap).Parse(text)).Execute(pg.outputBuffer, data); err != nil {
		panic(err)
	}
}
